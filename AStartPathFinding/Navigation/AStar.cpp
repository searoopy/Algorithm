...
std::vector<Node*> AStar::FindPath(Node* startNode, Node* goalNode, std::vector<std::vector<int>>& grid)
{
    ...
    {
        //현재 열린 리스트에 있는 노드중 fcost가 가장 낮은 노드를 검색.
        ...
        for (const Direction& direction : directions)
        {
            ...
            //이웃 노드가 열린 리스트에 있으면 더 좋은 비용일때만 처리.
            if (openListNode)
            {
                //비용 확인
                if(neighborNode->gCost < openListNode->gCost)
                {
                    //더 좋은 비용이면 열린 리스트의 노드 업데이트.
...
void AStar::DisplayGridWithPath(std::vector<std::vector<int>>& grid, const std::vector<Node*>& path)
{
    ...
    {
        //경로는 "*" 로 표시.
		position.X = static_cast<short>(node->position.x * 2); //각 숫자 사이에 공백이 있으므로 x좌표는 2배로.    
